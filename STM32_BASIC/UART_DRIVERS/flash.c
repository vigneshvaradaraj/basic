...
#include "stm32f4xx.h"  // Include the device header for STM32F4 family
...
void Flash_WriteData(uint32_t Address, uint32_t *Data, uint32_t Length)
{
    ...
    for (uint32_t i = 0; i < Length; i++)
    {
        ...
        *(__IO uint32_t*)(Address + i * 4) = Data[i]; // Write data to address
...
int main(void)
{
    ...
    // Write data to Flash
    Flash_WriteData(FLASH_SECTOR7_BASE_ADDR, data_to_write, 2);

    // Read data from Flash
    Flash_ReadData(FLASH_SECTOR7_BASE_ADDR, data_read, 2);
